...
app.whenReady().then(async()=>{
  ...
  try{
    ...
    stage='point';assert.equal(await send(1,{type:'point',x:100/1100,y:45/800,click:true}),true);
    stage='text';await delay(50);assert.equal(await send(2,{type:'text',text:'fixture@example.invalid'}),true);
    // Fixture-only DOM inspection; never used on a provider page.
    stage='read-text';const value=await wc.mainFrame.executeJavaScript('document.querySelector("#email").value');assert.equal(value,'fixture@example.invalid');
    stage='erase';await delay(50);assert.equal(await send(3,{type:'erase'}),true);
    stage='read-erased';assert.equal(await wc.mainFrame.executeJavaScript('document.querySelector("#email").value'),'');
    ...
    stage='playback-return';await wc.mainFrame.executeJavaScript('history.pushState({},"","/watch/home")').catch(error=>{if(remote.active||!wc.isDestroyed())throw error;});await delay(100);

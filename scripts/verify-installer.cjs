...
fs.cpSync(path.join(installed,'runtime-a'),path.join(installed,'runtime-b'),{recursive:true});fs.writeFileSync(startup(migrated),`@echo off\r\nstart "" "${path.join(installed,'runtime-b','myREWRD TV Box.exe').toUpperCase()}"\r\n`);const previous=fs.readFileSync(startup(migrated));run(migrated);assert.deepEqual(fs.readFileSync(startup(migrated)),previous);

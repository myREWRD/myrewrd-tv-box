...
function allowedNavigation(value, apiBase, token) {
  try {
    ...
    if (url.hostname === "sp.auth.adobe.com") return /^\/api\/v2\/authenticate\/ESPN\/[A-Za-z0-9_-]+\/?$/.test(url.pathname)
      ...
      // Observed Hulu OAuth return during ESPN TV-provider authentication.
      || (url.pathname === "/adobe-services/oauth2" && ["code", "state"].every(key =>
        url.searchParams.getAll(key).length === 1 && /^[A-Za-z0-9._~-]{1,512}$/.test(url.searchParams.get(key))));

...
async function poll() {
  ...
  try {
    ...
    document.getElementById('device').textContent = data.device_name || 'myREWRD Demo TV';
    ...
    if (data.offer && !peer) {
      ...
      connection.ontrack = event => { video.srcObject = event.streams[0] || new MediaStream([event.track]); video.hidden = false; waiting.hidden = true; video.play().catch(() => show('failed', 'Unable to play. Relaunch Presentation Mode.')); };
      connection.onconnectionstatechange = () => {
        ...
        if (['failed','closed','disconnected'].includes(connection.connectionState)) {
          ...
          show('failed', 'Presenter disconnected. Relaunch Presentation Mode to reconnect.');
        }
      };

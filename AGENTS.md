...
Starting with 2.3.27, runtime and dashboard setup use one semantic release version. Do not introduce a separately displayed setup revision suffix. Changes to provisioning helpers require a coordinated versioned runtime/setup release and matching dashboard pins, even when player behavior is unchanged. Keep old assets immutable and distinguish TV software OTA from Windows/Google setup work.

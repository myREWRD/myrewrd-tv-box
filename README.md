...
Release blockers remain: older clients do not honor the new failed-version record, so legacy adoption needs a separately verified delivery/retry policy; terminating the supervisor itself after the old app exits is not protected by a surviving watchdog; physical sleep/power policy acceptance is pending. Neither a heartbeat nor a successful download proves the board is visible. Keep rollout paused until these boundaries and representative hardware acceptance are resolved.

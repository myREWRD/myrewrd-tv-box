...
# Destination Wi-Fi delivery (2.3.29 candidate)
...
This is a candidate, not an approved provisioning release. Physical saved-profile/current-connection and playback acceptance are required before promoting2.3.29;2.3.27 remains the approved installer until then. See the dashboard repository's `docs/security/TV_WIFI_DELIVERY.md` for backend authorization, private migration, expiry cleanup, limits and release procedure. No mobile update is required.

[VERIFIED 2026-09-23] The immutable 2.3.28 candidate failed physical Wi-Fi acceptance because Windows PowerShell required an explicit System.Xml assembly reference. Version 2.3.29 includes that fix and an actual Windows PowerShell compilation regression. Applying the identical helper correction on the home test box allowed dashboard delivery to save a destination network, retain all existing profiles, enable automatic connection and keep the existing connection active; the delivery envelope was cleared. Unmodified 2.3.29 signed-artifact/update and playback acceptance remain pending. No customer SSID/password is recorded here.

[OWNER APPROVED 2026-09-23] Owner approved2.3.29 after the signed installed updater reported that version and real Wi-Fi delivery was verified. Windows recovery/source/build CI passed (an unrelated private-window fixture passed on retry). The approved immutable ZIP/setup pair is published; the companion dashboard aligns provisioning and fleet publication. This supersedes the candidate status above. The destination network connection still requires on-site testing; no fresh explicit physical picture/sound confirmation was supplied with release approval.
